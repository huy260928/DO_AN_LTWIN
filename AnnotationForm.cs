...
namespace AppFixPicture
{
    ...
    public partial class AnnotationForm : Form
    {
        ...
        private Bitmap displayBitmap; // scaled image shown in pictureBox

        // preview container so image won't overlap toolstrip
        ...
        public AnnotationForm()
        {
            ...
            // Create previewPanel and host pictureBox inside it
            try
            {
                ...
                // remove pictureBox from form controls and add to previewPanel
                ...
                pictureBoxCanvas.Dock = DockStyle.None;
                previewPanel.Controls.Add(pictureBoxCanvas);

                // add previewPanel to form below toolstrip and above statusstrip
            ...
            try {
                // populate common font sizes so user can pick from list
                cbFontSize.Items.Clear();
                int[] sizes = new[] { 8, 9, 10, 11, 12, 14, 16, 18, 20, 24, 28, 32, 36, 48, 72 };
                foreach (var s in sizes) cbFontSize.Items.Add(s.ToString());
                cbFontSize.Text = currentFontSize.ToString();
            } catch { }

            // support direct text change in font size combobox
            try
            {
                if (cbFontSize != null && cbFontSize.ComboBox != null)
                {
                    cbFontSize.ComboBox.TextChanged += (s, e) => {
                        var tb = cbFontSize.ComboBox;
                    ...
                    // also handle pressing Enter to commit
                    cbFontSize.ComboBox.KeyDown += (s, e) => {
        ...
        private void WireEvents()
        {
            ...
            btnZoomIn.Click += (s, e) => { zoom += 0.1f; UpdateDisplay(); UpdateZoomDisplay(); };
            btnZoomOut.Click += (s, e) => { zoom = Math.Max(0.1f, zoom - 0.1f); UpdateDisplay(); UpdateZoomDisplay(); };
        ...
        private void UpdateDisplay()
        {
            ...
            if (baseBitmap == null) { pictureBoxCanvas.Image = null; return; }

            // Create scaled bitmap according to zoom and assign to pictureBox
            int w = Math.Max(1, (int)(baseBitmap.Width * zoom));
            int h = Math.Max(1, (int)(baseBitmap.Height * zoom));
            displayBitmap = new Bitmap(w, h);
            ...
            // ensure previewPanel scrolls if image larger than panel
            if (previewPanel != null)
            {
                previewPanel.AutoScroll = (displayBitmap.Width > previewPanel.ClientSize.Width || displayBitmap.Height > previewPanel.ClientSize.Height);
            }
        ...
        {
            if (baseBitmap == null || displayBitmap == null) return p;
            // p is relative to pictureBox top-left (events attached to pictureBox)
            int imgX = (int)(p.X / zoom);
            int imgY = (int)(p.Y / zoom);
        ...
        private Point ImageToControl(Point imgPt)
        {
            ...
            int x = (int)(imgPt.X * zoom);
            int y = (int)(imgPt.Y * zoom);
        ...
        private void PictureBoxCanvas_Paint(object sender, PaintEventArgs e)
        {
            ...
            // draw preview shapes and annotations on top
            ...
            foreach (var a in textAnnotations)
            {
                ...
                using (Font drawFont = new Font(a.Font.FontFamily, Math.Max(1f, a.Font.Size * zoom), a.Font.Style))
        ...
        private TextAnnotation GetAnnotationAt(Point controlPoint)
        {
            for (int i = textAnnotations.Count - 1; i >= 0; i--)
            {
                ...
                if (a == null) continue;
                var dispPt = ImageToControl(a.Location);
                using (var f = new Font(a.Font.FontFamily, Math.Max(1f, a.Font.Size * zoom), a.Font.Style))
                {
