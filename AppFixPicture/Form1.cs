...
namespace AppFixPicture
{
    public partial class Form1 : Form
    {
        ...
        public Form1(IUnitOfWork unitOfWork, IProjectService projectService, IEditHistoryService historyService)
        {
            ...
            // Wire Annotation menu item to open AnnotationForm and accept edited image on Apply
            this.rotateToolStripMenuItem.Click += (s, e) =>
            {
                try
                {
                    using (var frm = new AnnotationForm(currentImage))
                    {
                        ...
                        if (dr == DialogResult.OK && frm.EditedImage != null)
                        {
                            // Replace currentImage with edited image
                            if (currentImage != null) currentImage.Dispose();
                            currentImage = new Bitmap(frm.EditedImage);

                            // Update stored/original bitmaps
        ...
        // Added handler for Saturation reset button referenced by Designer (btnSaturation)
        private void btnSaturation_Click(object sender, EventArgs e)
        {
            // Reset saturation to neutral (100%) and update label
            if (trackBarSaturation != null)
            {
                try
                {
                    trackBarSaturation.Value = 100;
                }
                catch { /* ignore if not initialized in designer-time */ }
                lblSaturationValue.Text = $"Độ bão hòa: {trackBarSaturation.Value}%";
            }
            ApplyAdjustments();
        }
        ...
        private void btnResetTemperature_Click(object sender, EventArgs e)
        {
            ...
            lblTemperatureValue.Text = $"Nhiệt độ màu: 0";

            ApplyAdjustments();
        }

        // Designer event handlers that were missing - implement here
        private void cropToolStripMenuItem_Click(object sender, EventArgs e)
        {
            // Same behavior as earlier: open FormCrop dialog with current image
            if (pictureBox1.Image == null && originalLoadedBitmap == null)
            {
            ...
            using (FormCrop formCrop = new FormCrop())
            {
                Bitmap imageForCrop = null;
                if (pictureBox1.Image != null)
                {
                    imageForCrop = new Bitmap(pictureBox1.Image);
                }
                else if (originalLoadedBitmap != null)
                {
                    imageForCrop = new Bitmap(originalLoadedBitmap);
                }

                formCrop.ImageToCrop = imageForCrop;
                var dr = formCrop.ShowDialog(this);
                if (dr == DialogResult.OK && formCrop.ResultImage != null)
                {
                ...
                {
                    try { formCrop.ImageToCrop?.Dispose(); } catch { }
                }
            }
        }
        ...
        private void btnCoolEffect_Click(object sender, EventArgs e)
        {
            ...
            trackBarTemperature.Value = -80;
            trackBarSaturation.Value = 110;
            trackBarBrightness.Value = 10;
            trackBarContrast.Value = 5;
            trackBarHue.Value = -5;
            lblTemperatureValue.Text = $"Nhiệt độ: {trackBarTemperature.Value}";
            lblSaturationValue.Text = $"Độ bão hòa: {trackBarSaturation.Value}%";
            lblBrightnessValue.Text = $"Độ sáng: {trackBarBrightness.Value}";
            lblContrastValue.Text = $"Tương phản: {trackBarContrast.Value}";
            lblHueValue.Text = $"Tông màu: {trackBarHue.Value}°";
            ApplyAdjustments();
            SaveState();
        }
        ...
        {
            if (originalStoredBitmap == null) return;
            trackBarSaturation.Value = 115;
            trackBarContrast.Value = 15;
            trackBarTemperature.Value = 10;
            trackBarBrightness.Value = 5;
            trackBarHue.Value = 0;
            lblSaturationValue.Text = $"Độ bão hòa: {trackBarSaturation.Value}%";
            lblContrastValue.Text = $"Tương phản: {trackBarContrast.Value}";
            lblTemperatureValue.Text = $"Nhiệt độ màu: {trackBarTemperature.Value}";
            lblBrightnessValue.Text = $"Độ sáng: {trackBarBrightness.Value}";
            ApplyAdjustments();
            SaveState();
        }
        ...
        {
            if (originalStoredBitmap == null) return;
            trackBarTemperature.Value = 60;
            trackBarSaturation.Value = 110;
            trackBarBrightness.Value = 5;
            trackBarContrast.Value = 5;
            trackBarHue.Value = 20;
            lblTemperatureValue.Text = $"Nhiệt độ: {trackBarTemperature.Value}";
            ApplyAdjustments();
            SaveState();
        }
        ...
        {
            if (originalStoredBitmap == null) return;
            // Apply faded by using existing ApplyFaded logic when available; mimic by adjusting slightly
            trackBarSaturation.Value = 110;
            trackBarBrightness.Value = 0;
            trackBarContrast.Value = 0;
            trackBarTemperature.Value = 0;
        ...
        {
            if (originalStoredBitmap == null) return;
            trackBarContrast.Value = 25;
            trackBarBrightness.Value = -10;
            trackBarSaturation.Value = 105;
            trackBarTemperature.Value = -10;
            trackBarHue.Value = 0;
            ApplyAdjustments();
            SaveState();
        ...
        {
            if (originalStoredBitmap == null) return;
            trackBarBrightness.Value = 40;
            trackBarContrast.Value = -10;
            trackBarTemperature.Value = 5;
            trackBarSaturation.Value = 105;
        ...
        private void btnVivid_Click(object sender, EventArgs e)
        {
            if (originalStoredBitmap == null) return;
            trackBarBrightness.Value = 10;
            trackBarContrast.Value = 35;
            trackBarTemperature.Value = 5;
            trackBarSaturation.Value = 130;
            trackBarHue.Value = 10;
            ApplyAdjustments();
            SaveState();
        }
        ...
        {
            if (originalStoredBitmap == null) return;
            trackBarBrightness.Value = 25;
            trackBarContrast.Value = -20;
            trackBarTemperature.Value = 30;
            trackBarSaturation.Value = 90;
            trackBarHue.Value = 5;
            ApplyAdjustments();
            SaveState();
        }
        ...
        {
            if (originalStoredBitmap == null) return;
            trackBarBrightness.Value = 15;
            trackBarContrast.Value = 20;
            trackBarTemperature.Value = 15;
            trackBarSaturation.Value = 110;
            trackBarHue.Value = 10;
            ApplyAdjustments();
            SaveState();
        ...
        private void btnSeaEffect_Click(object sender, EventArgs e)
        {
            ...
            trackBarContrast.Value = -20;
            trackBarTemperature.Value = -20;
            trackBarSaturation.Value = 80;
            trackBarHue.Value = -15;
            ApplyAdjustments();
            SaveState();
        }
        ...
        private void btnAutumnEffect_Click(object sender, EventArgs e)
        {
            ...
            trackBarBrightness.Value = -5;
            trackBarContrast.Value = 20;
            trackBarTemperature.Value = 25;
            trackBarSaturation.Value = 75;
            trackBarHue.Value = 15;
            ApplyAdjustments();
            SaveState();
        }
        ...
        private void btnSunlightEffect_Click(object sender, EventArgs e)
        {
            ...
            trackBarContrast.Value = -10;
            trackBarTemperature.Value = 40;
            trackBarSaturation.Value = 95;
            trackBarHue.Value = 0;
            ApplyAdjustments();
            SaveState();
        }
